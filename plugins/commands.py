...
async def accept(client, message):
    ...
    ts = await client.listen(message.chat.id)
    if ts.forward_from_chat and not ts.forward_from_chat.type in [enums.ChatType.PRIVATE, enums.ChatType.BOT]:
        chat_id = ts.forward_from_chat.id
        try:
